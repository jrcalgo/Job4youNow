...
const API_BASE = process.env.J4N_TELEGRAM_API_BASE || `https://${API_HOST}`;
...
export async function sendMessage(token, chatId, text, { replyMarkup, dryRun = false } = {}) {
...
export async function sendDocument(token, chatId, absPath, caption, { dryRun = false } = {}) {
  ...
  form.append('caption', escapeHtml(caption).slice(0, 1024));
