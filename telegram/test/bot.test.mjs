...
test('no active queue: HELP replies with HELP_TEXT even though nothing has been ingested', async () => {
  installBackend({ session: { chat_id: CHAT_ID, queue_id: null, status: 'idle', cursor: 1, telegram_offset: 0, stats: {} } });
  const state = await (async () => null)();
  const next = await handleUpdate(textUpdate(1, 'help'), { token: 'tok', chatId: CHAT_ID, dryRun: false, state });
  assert.equal(next, null);
  assert.match(lastSentText(), /Buttons do the same/);
});

test('no active queue: NEXT gets the "no active queue" hint, not a crash', async () => {
  installBackend({ session: { chat_id: CHAT_ID, queue_id: null, status: 'idle', cursor: 1, telegram_offset: 0, stats: {} } });
  await handleUpdate(textUpdate(1, 'next'), { token: 'tok', chatId: CHAT_ID, dryRun: false, state: null });
  assert.match(lastSentText(), /No active queue yet/);
});

test('QUEUES lists ingested queues even with no active queue', async () => {
  installBackend({
